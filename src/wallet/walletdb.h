...
public:
    ...
    bool WriteZwvrSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZwvrSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZwvrSeed_deprecated(uint256& seed);
    bool EraseZwvrSeed();
    bool EraseZwvrSeed_deprecated();

    bool WriteZwvrCount(const uint32_t& nCount);
    bool ReadZwvrCount(uint32_t& nCount);
