...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zwvrspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 wvr at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zwvr send
enum ZerocoinSpendStatus {
    Zwvr_SPEND_OKAY = 0,                            // No error
    Zwvr_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    Zwvr_WALLET_LOCKED = 2,                         // Wallet was locked
    Zwvr_COMMIT_FAILED = 3,                         // Commit failed, reset status
    Zwvr_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    Zwvr_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    Zwvr_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    Zwvr_TRX_CREATE = 7,                            // Everything related to create the transaction
    Zwvr_TRX_CHANGE = 8,                            // Everything related to transaction change
    Zwvr_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    Zwvr_INVALID_COIN = 10,                         // Selected mint coin is not valid
    Zwvr_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    Zwvr_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    Zwvr_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    Zwvr_SPENT_USED_Zwvr = 14,                      // Coin has already been spend
    Zwvr_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    Zwvr_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZwvrBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZwvrOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzwvrAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzwvrTracker> zwvrTracker;
    ...
    void setZWallet(CzwvrWallet* zwallet);
    CzwvrWallet* getZWallet();
    bool isZeromintEnabled();
    void setZwvrAutoBackups(bool fEnabled);
    ...
    /// Get 10000 wvr output and keys which can be used for the Masternode
    ...
    /** zwvr reset */
    boost::signals2::signal<void()> NotifyzwvrReset;
