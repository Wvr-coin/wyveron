                                                        ...
                                                        model(0)
{
    ...
    ui->labelTitle->setText("Select wvr Denominations to Spend");
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::wvr;
    ...
    QString toolTip4 = tr("Can vary +/- %1 uwvr per input.").arg(dFeeVary);
    ...
    for (PAIRTYPE(QString, std::vector<COutput>) coins : mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show wvr address. In tree mode, address is not shown again for direct wallet address outputs
