...
#include "qt/wvr/forms/ui_addresseswidget.h"
#include "qt/wvr/addresslabelrow.h"
#include "qt/wvr/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(wvrGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a wvr address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress wvrAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(wvrAdd.Get(), label.toUtf8().constData(),
                wvrAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
