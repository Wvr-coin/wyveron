...
#include "qt/wvr/privacywidget.h"
#include "qt/wvr/forms/ui_privacywidget.h"
#include "qt/wvr/qtutils.h"
#include "guiutil.h"
#include "qt/wvr/denomgenerationdialog.h"
#include "qt/wvr/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(wvrGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zwvr anonymizes your wvr by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zwvr or convert back to wvr"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 wvr ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zwvr");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zwvr");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zwvr");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zwvr");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zwvr");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zwvr");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zwvr");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zwvr");
    ...
    // Only Convert to wvr enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzwvr->setTitleClassAndText("btn-title-grey", "Total 0 zwvr");
    ui->btnTotalzwvr->setSubTitleClassAndText("text-subtitle", "Show denominations of zwvr owned.");
    ui->btnTotalzwvr->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select wvr outputs to mint into zwvr.");
    ...
    connect(ui->btnTotalzwvr, SIGNAL(clicked()), this, SLOT(onTotalZwvrClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zwvr");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of wvr to mint into zwvr"));
    }else{
        btnText = tr("Convert back to wvr");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zwvr to convert back into wvr"));
    }
...
void PrivacyWidget::onTotalZwvrClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzwvr->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzwvr->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zwvr is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zwvr, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zwvr").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zwvr minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZwvr(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zwvr converted back to wvr"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zwvr </b>";
    ...
    ui->btnTotalzwvr->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
