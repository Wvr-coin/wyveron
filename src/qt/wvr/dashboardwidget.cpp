...
#include "qt/wvr/dashboardwidget.h"
#include "qt/wvr/forms/ui_dashboardwidget.h"
#include "qt/wvr/sendconfirmdialog.h"
#include "qt/wvr/txrow.h"
...
DashboardWidget::DashboardWidget(wvrGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of wvr staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarewvr, "square-chart-wvr");
    setCssProperty(ui->labelSquarezwvr, "square-chart-zwvr");
    setCssProperty(ui->labelwvr, "text-chart-wvr");
    setCssProperty(ui->labelZwvr, "text-chart-zwvr");
    ui->labelZwvr->setVisible(false);
    ui->labelSquarezwvr->setVisible(false);
    ...
    ui->labelAmountZwvr->setText("0 zwvr");
    ui->labelAmountZwvr->setVisible(false);
    ui->labelAmountwvr->setText("0 wvr");
    setCssProperty(ui->labelAmountwvr, "text-stake-wvr-disable");
    setCssProperty(ui->labelAmountZwvr, "text-stake-zwvr-disable");
    ...
    ui->btnHowTo->setText(tr("How to get wvr"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("wvr")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool iswvr = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZwvr;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair wvr, zwvr
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            wvr = (pair.first != 0) ? pair.first / 100000000 : 0;
            zwvr = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalwvr += pair.first;
            chartData->totalZwvr += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valueswvr.append(wvr);
        chartData->valueszwvr.append(zwvr);

        int max = std::max(wvr, zwvr);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("wvr");
    set1 = new QBarSet("zwvr");
    set0->setColor(QColor(183, 90, 2));
    ...
    set0->append(chartData->valueswvr);
    set1->append(chartData->valueszwvr);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalwvr > 0 || chartData->totalZwvr > 0) {
        setCssProperty(ui->labelAmountwvr, "text-stake-wvr");
        setCssProperty(ui->labelAmountZwvr, "text-stake-zwvr");
    } else {
        setCssProperty(ui->labelAmountwvr, "text-stake-wvr-disable");
        setCssProperty(ui->labelAmountZwvr, "text-stake-zwvr-disable");
    }
    forceUpdateStyle({ui->labelAmountwvr, ui->labelAmountZwvr});
    ui->labelAmountwvr->setText(GUIUtil::formatBalance(chartData->totalwvr, nDisplayUnit));
    ui->labelAmountZwvr->setText(GUIUtil::formatBalance(chartData->totalZwvr, nDisplayUnit, true));
