...
#include "qt/wvr/send.h"
#include "qt/wvr/forms/ui_send.h"
#include "qt/wvr/addnewcontactdialog.h"
#include "qt/wvr/qtutils.h"
#include "qt/wvr/sendchangeaddressdialog.h"
#include "qt/wvr/optionbutton.h"
#include "qt/wvr/sendconfirmdialog.h"
#include "qt/wvr/myaddressrow.h"
#include "qt/wvr/guitransactionsutils.h"
...
#include "zwvrcontroldialog.h"

SendWidget::SendWidget(wvrGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("wvr");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zwvr");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (wvr)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a wvr address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 wvr");
    ...
    setCssProperty(coinIcon, "coin-icon-wvr");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onwvrSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onwvrSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZwvr = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZwvr));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZwvr ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendwvr = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendwvr ? AskPassphraseDialog::Context::Send_wvr : AskPassphraseDialog::Context::Send_zwvr, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendwvr) ? send(recipients) : sendZwvr(recipients)) {
...
bool SendWidget::sendZwvr(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zwvr is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zwvr selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZwvrControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZwvrControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZwvr(
        ...
        inform(tr("zwvr transaction sent!"));
        ZwvrControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == Zwvr_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zwvr require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zwvr transaction
        ...
        emit message("zwvr transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(iswvr){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any wvr to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZwvrControlDialog *zwvrControl = new ZwvrControlDialog(this);
            zwvrControl->setModel(walletModel);
            zwvrControl->exec();
            ui->btnCoinControl->setActive(!ZwvrControlDialog::setSelectedMints.empty());
            zwvrControl->deleteLater();
        } else {
            inform(tr("You don't have any zwvr in your balance to select."));
        }
    }
}
...
void SendWidget::onwvrSelected(bool _iswvr){
    iswvr = _iswvr;
    setCssProperty(coinIcon, _iswvr ? "coin-icon-wvr" : "coin-icon-zwvr");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress wvrAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(wvrAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(wvrAdd.Get(), dialog->getLabel().toStdString(), "send")) {
