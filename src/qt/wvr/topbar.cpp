...
#include "qt/wvr/topbar.h"
#include "qt/wvr/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/wvr/lockunlock.h"
#include "qt/wvr/qtutils.h"
#include "qt/wvr/receivedialog.h"
...
TopBar::TopBar(wvrGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopwvr, ui->labelAmountTopzwvr}, "amount-small-topbar");
    setCssProperty({ui->labelAmountwvr, ui->labelAmountzwvr}, "amount-topbar");
    setCssProperty({ui->labelPendingwvr, ui->labelPendingzwvr, ui->labelImmaturewvr, ui->labelImmaturezwvr}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzwvr->setVisible(false);
    ui->labelAmountzwvr->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzwvr->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezwvr->setVisible(false);
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount wvrAvailableBalance = balance + delegatedBalance;

    // zwvr Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalwvr = GUIUtil::formatBalance(wvrAvailableBalance, nDisplayUnit);
    QString totalzwvr = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopwvr->setText(totalwvr);
    ui->labelAmountTopzwvr->setText(totalzwvr);

    // Expanded
    ui->labelAmountwvr->setText(totalwvr);
    ui->labelAmountzwvr->setText(totalzwvr);

    ui->labelPendingwvr->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzwvr->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmaturewvr->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezwvr->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
