...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered wvr address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 wvr.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal wvr:URI (wvr:XyZ...?message=example)
            {
...
bool WalletModel::createZwvrSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZwvr(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzwvrReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzwvrReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzwvrReset.connect(boost::bind(NotifyzwvrReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzwvrReset.disconnect(boost::bind(NotifyzwvrReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid wvr address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid wvr address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from wvr address");
...
{
    setMints.clear();
    setMints = pwalletMain->zwvrTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
