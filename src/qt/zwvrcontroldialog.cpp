...
#include "ui_zwvrcontroldialog.h"
...
std::set<std::string> ZwvrControlDialog::setSelectedMints;
std::set<CMintMeta> ZwvrControlDialog::setMints;

bool CZwvrControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZwvrControlDialog::COLUMN_DENOMINATION || column == ZwvrControlDialog::COLUMN_VERSION || column == ZwvrControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZwvrControlDialog::ZwvrControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZwvrControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zwvr Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZwvr->setProperty("cssClass", "text-main-purple");
    ui->labelZwvr_int->setProperty("cssClass", "text-main-purple");
...
ZwvrControlDialog::~ZwvrControlDialog()
{
    delete ui;
}

void ZwvrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZwvrControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZwvrControlWidgetItem* itemDenom(new CZwvrControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZwvrControlWidgetItem *itemMint = new CZwvrControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zwvrTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zwvrTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zwvr.";
            else if (!mint.isSeedCorrect)
                strReason = "The zwvr seed used to mint this zwvr is not the same as currently hold in the wallet";
            else
...
void ZwvrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZwvrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZwvr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZwvrControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZwvrControlDialog::GetSelectedMints()
{
...
void ZwvrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
