...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::wvr);
    ...
    if (settings.contains("nAnonymizewvrAmount") || reset)
        SoftSetArg("-anonymizewvramount", settings.value("nAnonymizewvrAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in wvr.cpp during shotdown to also remove the window positions
