...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zwvrObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zwvrObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zwvrObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZwvr()) ? "Not available" : std::to_string(
...
UniValue getrawmempool(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in wvr\n"
...
UniValue getblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"zwvrsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zwvr denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zwvr denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zwvr denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zwvr denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zwvr denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zwvr denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zwvr denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zwvr denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zwvr denominations\n"
            "  },\n"
            "  \"CoinStake\" :\n"
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of wvr addresses\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), Zwvr_FAILED_ACCUMULATOR_INITIALIZATION);
...
UniValue getblockindexstats(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
                ...
                "  }\n"
                "  \"txbytes\": xxxxx                (numeric) Sum of the size of all txes (zwvr excluded) over block range\n"
                "  \"ttlfee\": xxxxx                 (numeric) Sum of the fee amount of all txes (zwvr mints excluded) over block range\n"
                "  \"ttlfee_all\": xxxxx             (numeric) Sum of the fee amount of all txes (zwvr mints included) over block range\n"
