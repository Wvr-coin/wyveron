...
extern UniValue DoZwvrSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzwvrseed(const UniValue& params, bool fHelp);
extern UniValue getzwvrseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzwvr(const UniValue& params, bool fHelp);
extern UniValue dzwvrstate(const UniValue& params, bool fHelp);
