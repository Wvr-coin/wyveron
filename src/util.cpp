...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "wvr" is a composite category enabling all wvr-related debug output
            if (ptrCategory->count(std::string("wvr"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\wvr
// Windows >= Vista: C:\Users\Username\AppData\Roaming\wvr
// Mac: ~/Library/Application Support/wvr
// Unix: ~/.wvr
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "wvr";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "wvr.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty wvr.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override wvr.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "wvrd.pid"));
