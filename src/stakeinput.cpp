...
CZwvrStake::CZwvrStake(const libzerocoin::CoinSpend& spend)
{
...
int CZwvrStake::GetChecksumHeightFromMint()
{
...
int CZwvrStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZwvrStake::GetChecksum()
{
    return nChecksum;
}

// The zwvr block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZwvrStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZwvrStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zwvr staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZwvrStake::GetUniqueness()
{
    //The unique identifier for a zwvr is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZwvrStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZwvrStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zwvr that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZwvrOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zwvr output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zwvr", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZwvrOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zwvr output", __func__);
...
bool CZwvrStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZwvrStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzwvrTracker* zwvrTracker = pwallet->zwvrTracker.get();
    CMintMeta meta;
    if (!zwvrTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zwvrTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!wvr Stake
bool CwvrStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CwvrStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CwvrStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CwvrStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CwvrStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CwvrStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CwvrStake::GetUniqueness()
{
    //The unique identifier for a wvr stake is the outpoint
...
CBlockIndex* CwvrStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
