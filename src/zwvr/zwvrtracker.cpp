// Copyright (c) 2018-2019 The wvr developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zwvr/deterministicmint.h>
#include "zwvrtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzwvrTracker::CzwvrTracker(std::string strWalletFile)
{
...
CzwvrTracker::~CzwvrTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzwvrTracker::Init()
{
...
bool CzwvrTracker::Archive(CMintMeta& meta)
{
...
bool CzwvrTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzwvrTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzwvrTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzwvrTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzwvrTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzwvrTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzwvrTracker::GetSerialHashes()
{
...
CAmount CzwvrTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzwvrTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzwvrTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzwvrTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzwvrTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzwvrTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzwvrTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzwvrTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzwvrTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzwvrTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzwvrTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzwvrWallet* zwvrWallet)
{
    bool iszwvrWalletInitialized = (NULL != zwvrWallet);
    CMintMeta meta;
    ...
    if (! iszwvrWalletInitialized)
        zwvrWallet = new CzwvrWallet(strWalletFile);
    meta.isSeedCorrect = zwvrWallet->CheckSeed(dMint);
    if (! iszwvrWalletInitialized)
        delete zwvrWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzwvrTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzwvrTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzwvrTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzwvrTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzwvrTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzwvrTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzwvrWallet* zwvrWallet = new CzwvrWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zwvrWallet);
        }
        delete zwvrWallet;
        LogPrint("zero", "%s: added %d dzwvr from DB\n", __func__, listDeterministicDB.size());
    }
