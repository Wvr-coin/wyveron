...
CzwvrWallet::CzwvrWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zwvr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZwvrSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZwvrSeed_deprecated()) {
                    LogPrintf("%s: Updated zwvr seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zwvr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zwvr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzwvrWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZwvrCount(nCountLastUsed);
    else if (!walletdb.ReadZwvrCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzwvrWallet::Lock()
{
    seedMaster = 0;
}

void CzwvrWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzwvrWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZwvr(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzwvrWallet::LoadMintPoolFromDB()
{
...
void CzwvrWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzwvrWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzwvrWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zwvrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzwvrWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZwvr(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zwvrTracker which also adds to database
    pwalletMain->zwvrTracker->Add(dMint, true);
...
void CzwvrWallet::SeedToZwvr(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzwvrWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzwvrWallet::GenerateDeterministicZwvr(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzwvrWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzwvrWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzwvrWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
