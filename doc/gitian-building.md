...
*Setup instructions for a gitian build of wvr Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the wvr
...
to the wvr GitHub release page.
...
- [Building wvr Coin](#building-wvr-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/wvr-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/wvr-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/wvr-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/wvr-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/wvr-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/wvr-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/wvr/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@wyveroncoin.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/wvr-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/wvr-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/wvr-osx-*-build.assert
```
